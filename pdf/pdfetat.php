 ...
 * "Etat" en fonction de paramètres dans la table om_etat
 *
 * @package openmairie_exemple
 * @version SVN : $Id: pdfetat.php 3027 2015-02-02 18:38:24Z fmichon $
 */
...
foreach ($editions as $key => $value) {
     ...
     */
    // On récupère l'enregistrement 'om_etat' de la collectivité en cours dans
    // l'état 'actif'
    $sql = " select * from ".DB_PREFIXE."om_etat ";
    $sql .= " LEFT JOIN ".DB_PREFIXE."om_logo ";
    $sql .= " ON om_etat.logo=om_logo.id ";
    $sql .= " LEFT JOIN ".DB_PREFIXE."om_requete ";
    $sql .= " ON om_etat.om_sql=om_requete.om_requete ";
    $sql .= " where om_etat.id='".$f->db->escapeSimple($obj)."' ";
    $sql .= " and om_etat.actif IS TRUE ";
    $sql .= " and om_etat.om_collectivite='".$_SESSION['collectivite']."' ";
    $res1 = $f->db->query($sql);
    ...
    if ($res1->numrows() == 0) {
        ...
        // On récupère l'enregistrement 'om_etat' de la collectivité de niveau
        // 2 dans l'état 'actif'
        $sql = " select * from ".DB_PREFIXE."om_etat ";
        $sql .= " LEFT JOIN ".DB_PREFIXE."om_logo ";
        $sql .= " ON om_etat.logo=om_logo.id ";
        ...
        $sql .= " where om_etat.id='".$f->db->escapeSimple($obj)."' ";
        $sql .= " and om_etat.actif IS TRUE ";
        $sql .= " and om_etat.om_collectivite='".($niveau == "" ? -1 : $niveau)."' ";
        $res1 = $f->db->query($sql);
        $f->addToLog("pdfetat.php: db->query(\"".$sql."\");", VERBOSE_MODE);
        $f->isDatabaseError($res1);
        // Si on obtient aucun résultat
        if ($res1->numrows() == 0) {
            // On libère le résultat de la requête précédente
            $res1->free();
            // On récupère l'enregistrement 'om_etat' de la collectivité de
